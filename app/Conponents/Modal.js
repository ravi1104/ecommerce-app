      ...
      <div className="bg-white overflow-y-auto h-3/5 w-3/4 rounded-lg p-8 flex flex-col justify-center">
        {products.map((product) => (
          <div key={product.id} className="mb-6 flex">
            ...
            <button className="bg-red-400 text-white px-2 py-2 ml-5 rounded-md hover:bg-red-600" onClick={() => {setRefresh(refresh+1); removeFromCart(product.id) }}>Remove</button>
          </div>
        ))}
